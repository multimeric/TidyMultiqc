...
test_that("We can parse the qualimap coverage histogram", {
  ...
  expect_true('qualimap_coverage_histogram.median' %in% colnames(report))
  # Check an actual median value is roughly where it should be
  expect_equal(
    report %>% filter(metadata.sample_id == 'P4107_1001') %>% pull(qualimap_coverage_histogram.median),
    35,
    tolerance = 1
  )
  expect_equal(nrow(report), n_samples)
...
test_that("We can parse the qualimap cumulative coverage histogram", {
  ...
  expect_true('qualimap_genome_fraction.%Q30' %in% colnames(report))
  # Check an actual median value is roughly where it should be
  expect_equal(
    report %>% filter(metadata.sample_id == 'P4107_1001') %>% pull('qualimap_genome_fraction.%Q30'),
    74.66,
    tolerance = 0.01
  )
  expect_equal(nrow(report), n_samples)
...
test_that("We can parse the snpeff bar chart", {
  ...
  expect_true(all(c(
    'snpeff_variant_effects_region.Exon',
    'snpeff_variant_effects_region.UTR 3 Prime',
    'snpeff_variant_effects_region.Splice Site Acceptor'
  ) %in% colnames(report)))
  expect_equal(
    report %>% filter(metadata.sample_id == 'P4107_1001') %>% pull('snpeff_variant_effects_region.Exon'),
    191454
  )
})
